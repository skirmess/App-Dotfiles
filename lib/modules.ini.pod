...
=head2 B<source_shift>

Instead of linking the full module to the home directory, only files in the
B<source_shift> directory and its subdirectories are linked.

=head2 B<target_shift>

The B<target_shift> path is prepended to every file when it is linked to the
...
useful if you're using someone elses repository where you have no influence
...
for pulling and pusing. Then we add the vim-airline repository, which is
...
the home directory
  ...
  target_shift = .vim/bundle/vim-airline
