...
sub main {
    ...
    my $obj_s = new_ok( $class, [ runtime => $runtime, name => $name, target_shift => path('shift2/.SHIFT 2') ] );
    ...
    is_deeply( $linkables, $expected, '... target_shift is ignored' );
    ...
    is_deeply( $linkables, $expected, '... target_shift is ignoed' );

    # source_shift
    $obj = new_ok( $class, [ runtime => $runtime, name => $name, source_shift => 'dir1' ] );

    $linkables = $obj->get_linkables(q{.});
    $expected  = [];
    is_deeply( $linkables, $expected, 'source_shift to an empty dir' );

    $test_ws->child('dir1/shift1.txt')->spew();

    $linkables = $obj->get_linkables(q{.});
    $expected  = [qw(shift1.txt)];
    is_deeply( $linkables, $expected, 'source_shift to a dir with a single file' );

    # source_shift on file
    $obj = new_ok( $class, [ runtime => $runtime, name => $name, source_shift => path('dir1/shift1.txt') ] );
    $exception = exception { $obj->get_linkables(q{.}) };
    isa_ok( $exception, 'App::Dotfiles::Error', 'get_linkables() throws an exception when run with a source_shift on a file' );
    like( $exception, qr{Not a directory: $test_ws}, '... with correct message' );

    # source_shift on a dir with perm 000
    $test_ws->child('dir2')->mkpath();
    chmod 0000, $test_ws->child('dir2');

    $obj = new_ok( $class, [ runtime => $runtime, name => $name, source_shift => 'dir2' ] );
    $exception = exception { $obj->get_linkables(q{.}) };
    note($exception);
    isa_ok( $exception, 'App::Dotfiles::Error', 'get_linkables() throws an exception when run with a source_shift on a dir without permissions' );
