...
sub main {
    ...
    #
    {
        ...
        is( $obj->_cmd_version(), undef, '... returns undef' );
        $log->contains_ok(qr{dotf version \d[.]\d+});
    ...
    #
    {
        ...
        is( $obj->main(),  undef, '... returns undef' );
        $log->contains_ok(qr{dotf version \d[.]\d+});
        $log->empty_ok('... no more logs');
...

    {
        ...
        my $exception = exception { $obj->main() };
        isa_ok( $exception, 'Git::Wrapper::Exception', '... throws an Git::Wrapper::Exception error with invalid repository' );
        is_deeply( \@ARGV, [], '... @ARGV is empty' );
...

    {
        ...
        $log->contains_ok(q{Updating config '.config'});
        $log->contains_ok(q{No modules configured in 'modules.ini'});
        $log->empty_ok();
    }
